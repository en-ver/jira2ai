...
High-level Markdown writes recognize the canonical Jira mention `[~accountId:<id>]` in issue descriptions, compatible rich-text field strings, comment add/update bodies, and worklog add/update comments. It creates one semantic ADF mention, and Jira may notify the referenced account. Escaped, malformed, code, link, and image forms remain text.
